...
public class DiagonalSequenceGenerator extends SequenceGenerator {
    ...
    public Set<Integer> goForSmaller(int id, Set<Integer> integerSet){
        if ( boardEdges.isTopEdge(id) || boardEdges.isLeftEdge(id) || counter == arbiter.getNoForWin()) {
            return integerSet;
        } else {
            int nextId = id - STEP;
    ...
    public Set<Integer> goForBigger(int id, Set<Integer> integerSet) {
        if ( boardEdges.isBottomEdge(id) || boardEdges.isRightEdge(id) || counter == arbiter.getNoForWin()) {
            return integerSet;
        } else {
            int nextId = id + STEP;
            integerSet.add(nextId);
